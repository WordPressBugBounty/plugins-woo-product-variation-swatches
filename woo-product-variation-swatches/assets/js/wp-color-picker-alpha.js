(() => {//#region src/js/wp-color-picker-alpha.js
/**!
...
*/
(function($) {
	if ($.wp.wpColorPicker.prototype._hasAlpha) return;
	var image = "data:image/png;base64,", _after = "<div class=\"wp-picker-holder\" />", _wrap = "<div class=\"wp-picker-container\" />", _button = "<input type=\"button\" class=\"button button-small\" />", _deprecated = wpColorPickerL10n.current !== void 0;
	if (_deprecated) var _before = "<a tabindex=\"0\" class=\"wp-color-result\" />";
	else var _before = "<button type=\"button\" class=\"button wp-color-result\" aria-expanded=\"false\"><span class=\"wp-color-result-text\"></span></button>", _wrappingLabel = "<label></label>", _wrappingLabelText = "<span class=\"screen-reader-text\"></span>";
	/**
	* Overwrite Color
	...
	$.widget("wp.wpColorPicker", $.wp.wpColorPicker, {
		...
		*/
		_create: function() {
			...
			if (self.options.type === "hue") return self._createHueOnly();
			...
			if (self.options.defaultColor) {
				...
				if (!_deprecated) self.button.attr("aria-label", wpColorPickerL10n.defaultAriaLabel);
			} else {
				self.button.addClass("wp-picker-clear").val(wpColorPickerL10n.clear);
				if (!_deprecated) self.button.attr("aria-label", wpColorPickerL10n.clearAriaLabel);
			}
			if (_deprecated) el.wrap("<span class=\"wp-picker-input-wrap\" />").after(self.button);
			else {
				...
				*/
				change: function(event, ui) {
					if (self.options.alpha) {
						...
						if (_deprecated) self.toggler.html("<span class=\"color-alpha\" />");
						else {
							self.toggler.css({ "position": "relative" });
							if (self.toggler.find("span.color-alpha").length == 0) self.toggler.append("<span class=\"color-alpha\" />");
						}
		...
		_addListeners: function() {
			...
			*/
			self.toggler.click(function() {
				if (self.toggler.hasClass("wp-picker-open")) self.close();
				else self.open();
			});
			/**
			...
			*/
			self.element.on("change", function(event) {
				if ($(this).val() === "" || self.element.hasClass("iris-error")) {
					if (self.options.alpha) {
						if (_deprecated) self.toggler.removeAttr("style");
			...
			*/
			self.button.on("click", function(event) {
				...
				} else if ($(this).hasClass("wp-picker-default")) self.element.val(self.options.defaultColor).change();
			});
		}
	});
	/**
	...
	$.widget("a8c.iris", $.a8c.iris, {
		_create: function() {
			...
			if (typeof this.options.alpha !== "undefined" && this.options.alpha) {
				var self = this, el = self.element, aContainer = $("<div class=\"iris-strip iris-slider iris-alpha-slider\"><div class=\"iris-slider-offset iris-slider-offset-alpha\"></div></div>").appendTo(self.picker.find(".iris-picker-inner")), controls = {
					aContainer,
					aSlider: aContainer.find(".iris-slider-offset-alpha")
				};
				if (typeof el.data("custom-width") !== "undefined") self.options.customWidth = parseInt(el.data("custom-width")) || 0;
				else self.options.customWidth = 100;
		...
		_change: function() {
			...
			if (el.data("reset-alpha") || false) self.picker.find(".iris-palette-container").on("click.palette", ".iris-palette", function() {
		...
		_addInputListeners: function(input) {
			...
			if (self.options.hide) input.on("focus", function() {
				self.show();
			});
		}
	});
